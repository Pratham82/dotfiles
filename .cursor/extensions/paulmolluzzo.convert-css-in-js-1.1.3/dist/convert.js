// copied from https://github.com/ansumanshah/css-in-js
// MIT
...
const getBeginningWhitespace = string => string.match(/^\s+/) !== null ? string.match(/^\s+/)[0] : '';

const isCSS = item => (item.match(/;/g) || []).length === 1;

const isJS = item => (item.match(/,/g) || []).length === 1;

const toHyphen = prop => prop.replace(/([A-Z])/g, char => `-${char[0].toLowerCase()}`);

const toCamel = prop => prop.replace(/-(\w|$)/g, (dash, next) => next.toUpperCase());
